function Modelo_cluster3(varargin)
%%  S. Cervisiae Data
...
P.etoh0=0; %mM
P.NAD0=varargin{1};%20.7; %mM
P.aca0=0; %mM
P.NADH0=10.33; %mM
P.acetate0=0;%mM
P.ATP0=varargin{2};%20 ; %mM
P.CoA0=10 ; %mM
P.AcetylCoa0=0;  %mM
P.Bif0=0.8;   %mM
P.AMP0=10; %mM
...
P.CS_V_deletion=1; 
 ...
 P.Acs1_deletion=1;
 P.etoh=0.083;
 [tiempo,mets,flujos_r]=S(P,W,Z);
 csvwrite(string(varargin{1})+"_"+string(varargin{2})+"_"+"ref.csv",[tiempo mets(:,12)])
%  Test cit2p
          P.CS_V_deletion=0;
           P.MS_deletion=1;
           P.Acs1_deletion=1;
  ...
  csvwrite(string(varargin{1})+"_"+string(varargin{2})+"_"+"cit2.csv",[tiempo_mut mets_mut(:,12)])
 ...
 %  %% Test second mutation mls
    ...
    csvwrite(string(varargin{1})+"_"+string(varargin{2})+"_"+"mls1.csv",[tiempo_mut2 mets_mut2(:,12)])
...
%            csvwrite(string(varargin{1})+"_"+string(varargin{2})+"_"+"acs1p.csv",[tiempo_mut3 mets_mut3(:,12)])
...
function [tiempo,variables,dvariables]=S(P,W,Z)
    ...
    tiempoInicio = tic;
    opciones= odeset("RelTol",1e-3,"AbsTol",1e-6,'Events', @(t, y) eventoTiempoEjecucion(t, y, tiempoInicio));
...
function [value, isterminal, direction] = eventoTiempoEjecucion(t, y, tiempoInicio)
    tiempoMaximoEjecucion = 120;  % Tiempo máximo de ejecución en segundos
    tiempoActual = toc(tiempoInicio);  % Tiempo actual de ejecución
    value = tiempoMaximoEjecucion - tiempoActual;
    isterminal = 1;  % Detener la integración si se cumple la condición
    direction = 0;   % Detectar todos los ceros
end
end
end
